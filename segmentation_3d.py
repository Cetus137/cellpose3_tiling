...
def segment_3D_stack(image_stack, config, view):
    ...
    if view == 'XY':
        ...
        masks, flows, styles = model.eval(image_stack, diameter=None, z_axis = 0, channels=[0, 0],batch_size=config['batch_size'], do_3D=False, min_size=config['min_size'])
        #print(flows)
        print(len(flows))
        for flow in flows:
            print(flow.shape)

        flowsx_stack[:, :, :] = flows[1][1]
        flowsy_stack[:, :, :] = flows[1][0]
        cell_prob_stack[:, :, :] = flows[2]

        tiff.imwrite('cellprob_xy_raw_3d.tif', cell_prob_stack.astype(np.float32))

    elif view == 'XZ':
        masks, flows, styles = model.eval(image_stack, diameter=None, z_axis = 0, channels=[0, 0],batch_size=config['batch_size'], do_3D=False, min_size=config['min_size'])

        flowsx_stack[:, :, :] = flows[1][1]
        flowsz_stack[:, :, :] = flows[1][0]
        cell_prob_stack[:, :, :] = flows[2]
